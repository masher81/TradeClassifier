...
entries = tr[tr.action=='ENTER'].rename(columns={'time':'entry_time','price':'entry_price'})
exits   = tr[tr.action=='EXIT'].rename(columns={'time':'exit_time','price':'exit_price','pnl':'pnl'})
df = pd.merge(entries, exits[['symbol','trade_id','exit_time','exit_price','pnl']],
              on=['symbol','trade_id'], how='inner')
...
# ─────────────── OVERLAY BACKTEST ───────────────────────────────────────
feat_df['pred_proba'] = clf.predict_proba(feat_df.drop(['symbol','pnl','y'],1))[:,1]
